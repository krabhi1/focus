...
static void poll_idle_state(AppState *app) {
  ...
  if (now_idle && !app->is_idle) {
    emit_event(app, EVENT_KIND_IDLE, EVENT_STATE_ENTERED, "idle", "entered");
    app->is_idle = true;
...
int main(int argc, char *argv[]) {
  ...
  app.idle_poll_seconds =
      (argc - argi == 2) ? (unsigned int)strtoul(argv[argi + 1], NULL, 10) : 1U;
  ...
  emit_event(&app, EVENT_KIND_LISTENER, EVENT_STATE_READY, "listener", "ready");
  poll_idle_state(&app);
